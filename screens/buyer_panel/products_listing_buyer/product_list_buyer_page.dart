...
class _ListinguserPageState extends State<ListinguserPage>
  ...
  // 2. Add state for saved searches
  List<SavedSearchModel> _savedSearches = [];
  SavedSearchModel? _selectedSavedSearch;

  // 1. Add state for loading/error
  bool _isSavedSearchesLoading = false;
  ...
  void initState() {
    ...
    // Fetch saved searches (now safe, context is under BlocProvider)
            ...
            FadeTransition(
              ...
              child: SlideTransition(
                ...
                child: DiamondFilterPanel(
                  key: ValueKey(selectedFilters.hashCode),
  ...
  // 2. Refactor _buildHeader to use BlocConsumer and InputDecorator for the saved search dropdown
          ...
          return Container(
            ...
            child: Row(
              ...
              children: [
                ...
                BlocConsumer<SavedSearchBloc, SavedSearchState>(
                  listener: (context, state) {
                    if (state is SavedSearchLoading) {
                      setState(() {
                        _isSavedSearchesLoading = true;
                        _savedSearchesError = null;
                      });
                    } else if (state is SavedSearchLoaded) {
                      setState(() {
                        _savedSearches = state.searches;
                        _isSavedSearchesLoading = false;
                        _savedSearchesError = null;
                      });
                    } else if (state is SavedSearchError) {
                      setState(() {
                        _isSavedSearchesLoading = false;
                        _savedSearchesError = state.message;
                      });
                    }
                  },
                  builder: (context, state) {
                    return Row(
                      children: [
                                  ...
                                  : InputDecorator(
                                      decoration: InputDecoration(
                                        prefixIcon: const Icon(Icons.search,
                                            color: Colors.grey),
                                        suffixIcon: _selectedSavedSearch != null
                                            ? IconButton(
                                                icon: const Icon(Icons.close,
                                                    size: 18),
                                                tooltip: 'Clear saved filter',
                                                onPressed: () {
                                                  setState(() {
                                      ...
                                      child: DropdownButtonHideUnderline(
                                        child:
                                            DropdownButton<SavedSearchModel?>(
                                          isExpanded: true,
                                          value: _selectedSavedSearch,
                                          hint: const Text(
                                          ...
                                          onChanged: (selected) {
                                            setState(() {
                                              _selectedSavedSearch = selected;
                                              if (selected != null) {
                                                selectedFilters = {
                                                  if ((selected.filters['shape']
                                                              as List?)
                                                          ?.isNotEmpty ??
                                                      false)
                                                    'Shape': List<String>.from(
                                                        selected.filters[
                                                                'shape'] ??
                                                            []),
                                                  if ((selected.filters['carat']
                                                              as List?)
                                                          ?.isNotEmpty ??
                                                      false)
                                                    'Carat': List<String>.from(
                                                        selected.filters[
                                                                'carat'] ??
                                                            []),
                                                  if ((selected.filters['color']
                                                              as List?)
                                                          ?.isNotEmpty ??
                                                      false)
                        ...
                        _buildSearchAndFilters(theme, isWide: true),
                      ],
                    );
                  },
                ),
                  ...
                  builder: (context, state) {
                    return Column(
                      ...
                      children: [
                        ...
                        _buildSearchAndFilters(theme, isWide: false),
                      ],
                    );
                  },
                ),
  ...
  Widget _buildSearchAndFilters(ThemeData theme, {required bool isWide}) {
    if (isWide) {
      return Row(
